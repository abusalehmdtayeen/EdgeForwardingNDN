...
{
  int runLenArg, dotraceArg, aConfigId, rateArg, cfg_id_arg, sim_id; 
  std::string t_id, agPortId, rate, cfg_id, port;
  CommandLine cmd;
  
  cmd.AddValue ("tId", "topology ID", t_id);
  cmd.AddValue ("sId", "Simulation ID", sim_id);
  cmd.AddValue("agCnfg", "agent config ID", aConfigId);  
  cmd.AddValue("agPort", "agent port", port); 
  ...
  cmd.AddValue ("cfgId", "config file id", cfg_id_arg);
  cmd.Parse(argc, argv);
  ...
  cfg_id = std::to_string(cfg_id_arg);
  std::string resultID = "results/IsrPrML";
  std::string configFileName = t_id+"-cons-prod-config-" + cfg_id + ".txt";
  ...
  for (vector<std::string>::iterator it = agent_names.begin(); it != agent_names.end(); ++it)
  {
      StrategyChoiceHelper::Install<nfd::fw::IsrPr51MLStrategy>(Names::Find<Node>(*it),  prefix);
  }
...
void send_terminal(std::string port)
{
    ...
    std::cout << "Sending terminal signal …" << std::endl;
    socket.connect ("tcp://127.0.0.1:"+port);
    
    edgmlfw3msg::Node node;
    node.set_id("1111");
	//perfcmsgq::::Node::Face* face; 
    // if there is reward
...
map<std::string, std::string> read_config(std::string t_id, std::string int_rate, std::string configFileName){
  ...
  if (int_rate != "0")
    consumerHelper.SetAttribute("Frequency", StringValue(int_rate)); //set the rate from argument value
  ...
  while (!topgen.eof()){
     ...
     if (int_rate == "0") //set the rate from config file
        consumerHelper.SetAttribute("Frequency", StringValue(rate));
