...
{
  int runLenArg, dotraceArg, aConfigId; 
  std::string t_id, rateArg;
  CommandLine cmd;
  ...
  int sim_id = 1;
  std::string resultID = "train-traces";
  ...
  pConfig = read_config(t_id, rateArg, configFileName);
  ...
  for (vector<std::string>::iterator it = agent_names.begin(); it != agent_names.end(); ++it)
  {
      StrategyChoiceHelper::Install<nfd::fw::RandTrainStrategy>(Names::Find<Node>(*it),  prefix);
  }
