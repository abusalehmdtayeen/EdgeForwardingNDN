...
def get_plot_df(file_name, node_id, method, metric_type):
	...
	w_df.to_csv(curr_path + "/" + trace_id + "/" + out_dir + "/plot_data/"+ method + "-" + node_id + "-" + metric_type + ".csv", index=False)
...
def plot_data(plot_df, node_id, delay_type):
	...
	for idx, mth in enumerate(methods):
		plot_df.plot(kind='line', x='Time', y=node_id + "-" + mth + "-" + "Delay", color=colors[idx], lw=2, marker=markers[idx], markersize=marker_sizes[idx], label=mth, ax=ax)
		max_delays.append(plot_df[node_id + "-" + mth + "-" + "Delay"].max()/1000.0)
...
def plot_cdf_data(node_id, cdf_dict, delay_type):
	...
	for idx, mth in enumerate(methods): 
		...
		ax = plot_df.plot.hist(ax=ax, bins=n_bins, cumulative=True, density=True, histtype='step', label=mth, alpha=1.0, color=colors[idx], linewidth=2, legend=False)
...
if __name__ == "__main__":
	...
	parser.add_argument('-l', '--list',
                    ...
                    required=False,
                    help='comma or space delimited list of characters')  ## python3 test.py -l "abc xyz, 123"	
	...
	for ix, dl_type in enumerate(delay_types):
		...
		for node_id in nodes:
			for mth in methods:
				columns.append(node_id + "-" + mth + "-" + "Delay")
		...
		for idx, node_id in enumerate(nodes):
			...
			for mth in methods:
				...
				plot_df, dl_cdf = get_plot_df(file_name, node_id, mth, dl_type)
				cdf_dict[mth] = dl_cdf
				if not col_added:
					comp_df[['Time']] = plot_df[['Time']]
					col_added = True

				comp_df[[node_id + "-" + mth + "-" + "Delay"]] = plot_df[['AvgDelay']]
