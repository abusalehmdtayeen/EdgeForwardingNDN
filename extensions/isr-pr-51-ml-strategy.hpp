...
class IsrPr51MLStrategy : public Strategy {
public:
  IsrPr51MLStrategy(Forwarder& forwarder, const Name& name = getStrategyName());

  virtual ~IsrPr51MLStrategy() override;
  ...
  uint16_t send_msg(std::string node_name, std::vector<uint64_t> face_ids, std::vector<double> isr_pr_values, std::vector<double> delay_values, double ext_ft1, double ext_ft2);
  ...
  bool can_change(uint64_t max_face_id, uint64_t min_face_id);
...
protected:
  ...
  double thrpt_thrs = 0.95;
  double sucr_diff_thrs = 0.05; //succ_ratio diff threshold
  double delr_diff_thrs = 0.02; //succ_ratio diff threshold
  
  //double delr_diff_thrs = 0.15; //normalized delay ratio diff threshold
  double max_face_probability, min_face_probability = 0.01;
  std::vector<uint64_t> last_prob_faces;
  bool converge = false;
