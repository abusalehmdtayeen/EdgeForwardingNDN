...
#include "rand_train_strategy.hpp"
...
NFD_LOG_INIT("RandTrainStrategy");

namespace nfd {
namespace fw {

RandTrainStrategy::RandTrainStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
{
  ...
  this->file_path = base_path + "train-traces/";
}

RandTrainStrategy::~RandTrainStrategy()
{
}
...
void RandTrainStrategy::init( const fib::NextHopList& nexthops, std::string flow_id){
...
void RandTrainStrategy::set_sending_time(uint32_t nonce, uint64_t sel_face_id){
...
void
RandTrainStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                                 const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  if (isRandom) {
  	 ...
  	 do {
  	  ...
  	  } while(current_face_id == in_face_id);
  }
  else{
  ...
  if (this->m_fit.count(out_flow_id) == 0) { //the flow-face id does not exist in map    
      RandTrainStrategy::FaceInfo fc_info;
...
void RandTrainStrategy::write_vectors( std::string node_id)
{
...
void RandTrainStrategy::write_interest(std::string node_id, std::string interest_name, uint32_t nonce, uint64_t fid, bool isRt)
{
    std::ofstream outputFile;
...
void RandTrainStrategy::write_data(std::string node_id, std::string data_name, uint32_t int_nonce ,uint32_t data_nonce, uint64_t fid, double delay)
{
    std::ofstream outputFile;
...
void RandTrainStrategy::write_counts(std::string node_id)
{
    std::ofstream outputFile;
...
/*---------------------------------*/
void RandTrainStrategy::beforeSatisfyInterest(const shared_ptr< pit::Entry>& pitEntry, const Face& inFace,
  	                    const Data&  data)
{
	//----getting the node name-----
    ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
...
void
RandTrainStrategy::beforeExpirePendingInterest(const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  //std::cout << "MY strategy in node " << name << ": beforeExpirePendingInterest pitEntry=" << pitEntry->getName() << std::endl;
...
/*----------------------------------*/
const Name&
RandTrainStrategy::getStrategyName()
{
