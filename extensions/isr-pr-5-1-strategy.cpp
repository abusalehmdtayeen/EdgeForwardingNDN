...
NFD_LOG_INIT("IsrPr51Strategy");

namespace nfd {
namespace fw {

IsrPr51Strategy::IsrPr51Strategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
{
...
IsrPr51Strategy::~IsrPr51Strategy()
{
...
void IsrPr51Strategy::init( const fib::NextHopList& nexthops, std::string flow_id){
    ...
    for (selected = nexthops.begin(); selected != nexthops.end(); ++selected) { 
        ...
        uint32_t capacity = floor((bitrate * this->bw_mulp)/ this->pktSize) ; //link bandwidth in terms of pkts (maximum pkts)  //ceil
...
uint64_t IsrPr51Strategy::findLatestSendFaceId(const shared_ptr<pit::Entry>& pitEntry)
{
...
uint64_t IsrPr51Strategy::get_num_hops_prefix(const fib::NextHopList& nexthops)
{
...
void IsrPr51Strategy::set_sending_time(uint32_t nonce, uint64_t sel_face_id){
...
std::vector<std::pair<FaceId, double> > IsrPr51Strategy::sort_map(std::map<FaceId, double>& M) { 
...
void
IsrPr51Strategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                                 const shared_ptr<pit::Entry>& pitEntry)
{
     ...
     if (isRandom) { 
        ...
        {  
            ...
            if (this->m_fit.count(face_flow_id) == 0) { //the flow-face id does not exist in map    
                IsrPr51Strategy::FaceInfo fc_info;
  ...
  if (this->pkt_snd_count >= this->pkt_snd_thrs) //needs to write face statistics and reset
  {
	    ...
	    vector<uint64_t> isr_sorted_faces, del_sorted_faces;
        ...
        std::cout << node_name << std::endl;
        std::cout << "=======" << ns3::Simulator::Now().ToDouble(ns3::Time::S) << "S ===" << std::endl;
        ...
        {       
            ...
            {  
                ...
                if (sat_diff > sucr_diff_thrs && change_prob){ 
                    ...
                    std::cout << "Changed based on succ ratio" << std::endl;
                }
                else{
            ...
            /*if (not this->converge && (isr_sorted_faces.size()%2 != 0))
                prob_sorted_faces.push_back(isr_sorted_faces.at(j));*/

        } //end of thrpt condition
        else if (curr_thrpt >= thrpt_thrs) //check for delay
        {       
            std::cout << "~~~~~~~" << total_data_count << " " << curr_thrpt << " " << total_interest_count << "~~~~~~~~~" << std::endl;
            ...
            {     
                ...
                std::cout << "Max: " << max_face_flow_id << " " << max_prob << " " << "Min: " << min_face_flow_id << " " << min_prob << " " << del_ratio_diff << std::endl;
                
                bool change_prob = can_change(curr_face_id, sel_face_id);
                std::cout << "Can change: " << change_prob << std::endl;
                
                if (del_ratio_diff > delr_diff_thrs && change_prob){ 
                    new_prob = min_prob + this->pth;
                    new_prob = std::min(res_prob, new_prob);
                    min_fc_info.set_prob(new_prob); 
                    ...
                    std::cout << "Changed based on delay ratio" << std::endl;
                }
                else{
                    new_prob = std::min(res_prob, min_prob);
                    min_fc_info.set_prob(new_prob);
            ...
            /*if (not this->converge && (del_sorted_faces.size()%2 != 0))
...
bool IsrPr51Strategy::can_change(uint64_t max_face_id, uint64_t min_face_id)
{
      std::map<uint64_t, double> face_prob_values;
      double prob;
      std::cout << "!!!Inside can_change!!!" << std::endl;
      for (std::vector<uint64_t>::iterator it = this->flow_next_hops[this->src_flow].begin(); it != this->flow_next_hops[this->src_flow].end(); ++it)
      {  
            ...
            std::cout << sel_face_id << " " << prob << std::endl;
      }
      ...
      if ( (face_prob_values[max_face_id] > this->max_face_probability) || (face_prob_values[min_face_id] < this->min_face_probability) ){
        std::cout << "Face prob limit exceeded.." << std::endl;
        return false;
      }
...
//write model output
void IsrPr51Strategy::write_vectors(std::string node_id)
{
...
void IsrPr51Strategy::write_allocations(std::string node_id)
{
    std::ofstream outputFile;
...
void IsrPr51Strategy::write_interest(std::string node_id, std::string interest_name, uint32_t nonce, uint64_t fid, bool isRt)
{
    std::ofstream outputFile;
...
void IsrPr51Strategy::write_data(std::string node_id, std::string data_name, uint32_t int_nonce ,uint32_t data_nonce, uint64_t fid, double delay)
{
    std::ofstream outputFile;
...
void IsrPr51Strategy::write_counts(std::string node_id)
{
    std::ofstream outputFile;
...
void IsrPr51Strategy::beforeSatisfyInterest(const shared_ptr< pit::Entry>& pitEntry, const Face& inFace,
  	                    const Data&  data)
{
    //----getting the node name-----
    ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
...
//-------------------------------------------------------
void
IsrPr51Strategy::beforeExpirePendingInterest(const shared_ptr<pit::Entry>& pitEntry)
{
  ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  std::string node_name = ns3::Names::FindName(node);
...
const Name&
IsrPr51Strategy::getStrategyName()
{
  static Name strategyName("ndn:/localhost/nfd/strategy/isr-pr-5-1-strategy/%FD%01");
  return strategyName;
}
