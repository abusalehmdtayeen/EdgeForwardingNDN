...
#include "isr-pr-4-ml-strategy.hpp"
#include "FnvHash.hpp"
...
NFD_LOG_INIT("IsrPr4MLStrategy");

namespace nfd {
namespace fw {

IsrPr4MLStrategy::IsrPr4MLStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
{
...
IsrPr4MLStrategy::~IsrPr4MLStrategy()
{
...
void IsrPr4MLStrategy::init( const fib::NextHopList& nexthops, std::string flow_id){
...
uint64_t IsrPr4MLStrategy::findLatestSendFaceId(const shared_ptr<pit::Entry>& pitEntry)
{
...
uint64_t IsrPr4MLStrategy::get_num_hops_prefix(const fib::NextHopList& nexthops)
{
...
void IsrPr4MLStrategy::set_sending_time(uint32_t nonce, uint64_t sel_face_id){
...
std::vector<std::pair<FaceId, double> > IsrPr4MLStrategy::sort_map(std::map<FaceId, double>& M) { 
...
void
IsrPr4MLStrategy::afterReceiveInterest(const Face& inFace, const Interest& interest,
                                                 const shared_ptr<pit::Entry>& pitEntry)
{
     ...
     if (isRandom) { 
        ...
        {  
            ...
            if (this->m_fit.count(face_flow_id) == 0) { //the flow-face id does not exist in map    
                IsrPr4MLStrategy::FaceInfo fc_info;
  ...
  if (this->pkt_snd_count >= this->pkt_snd_thrs) //needs to write face statistics and reset
  {
        ...
        uint16_t cong_status = send_msg(node_name, face_ids, pr_avg_satr_values, delay_values, succ_diff);
...
//write model output
void IsrPr4MLStrategy::write_vectors(std::string node_id)
{
...
void IsrPr4MLStrategy::write_allocations(std::string node_id)
{
    std::ofstream outputFile;
...
void IsrPr4MLStrategy::write_interest(std::string node_id, std::string interest_name, uint32_t nonce, uint64_t fid, bool isRt)
{
    std::ofstream outputFile;
...
void IsrPr4MLStrategy::write_data(std::string node_id, std::string data_name, uint32_t int_nonce ,uint32_t data_nonce, uint64_t fid, double delay)
{
    std::ofstream outputFile;
...
void IsrPr4MLStrategy::write_counts(std::string node_id)
{
    std::ofstream outputFile;
...
void IsrPr4MLStrategy::beforeSatisfyInterest(const shared_ptr< pit::Entry>& pitEntry, const Face& inFace,
  	                    const Data&  data)
{
    //----getting the node name-----
    ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
...
//-------------------------------------------------------
void
IsrPr4MLStrategy::beforeExpirePendingInterest(const shared_ptr<pit::Entry>& pitEntry)
{
  ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  std::string node_name = ns3::Names::FindName(node);
...
//===========================================
uint16_t IsrPr4MLStrategy::send_msg(std::string node_name, std::vector<uint64_t> face_ids, std::vector<double> isr_pr_values, std::vector<double> delay_values, double ext_ft)
{
    ...
    edgmlfw2msg::Node node;
    node.set_id(node_name);
	edgmlfw2msg::Node::Face* face;
...
const Name&
IsrPr4MLStrategy::getStrategyName()
{
  static Name strategyName("ndn:/localhost/nfd/strategy/isr-pr-4-ml-strategy/%FD%01");
  return strategyName;
}
